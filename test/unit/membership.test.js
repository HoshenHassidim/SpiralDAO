...
describe("Membership", function () {
    ...
    describe("getMember", function () {
        it("Should return the username of a member", async function () {
            await membership.connect(deployer).registerMember("deployer")
            let username = await membership.getMember(deployerAddress)
            expect(username).to.equal("deployer")
        })

        it("Should revert when attempting to get username of non-member", async function () {
            await expect(membership.getMember(deployerAddress)).to.be.revertedWith("Not a member.")
        })
    })
})
